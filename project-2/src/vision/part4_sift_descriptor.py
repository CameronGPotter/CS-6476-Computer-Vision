...
def get_magnitudes_and_orientations(
    ...
    magnitudes = np.sqrt(np.multiply(Ix, Ix) + np.multiply(Iy, Iy))
...
def get_gradient_histogram_vec_from_patch(
    ...
    # bins = np.array([-7,-5,-3,-1,1,3,5,7]) * np.pi / 8
    wgh = np.zeros((128, 1))
    bins = 8
    r = (-1 * np.pi, np.pi)
    for i in range(0, 4):
        for j in range(0, 4):
            cells = window_orientations[4*i:4*i+4, 4*j:4*j+4]
            weights = window_magnitudes[4*i:4*i+4, 4*j:4*j+4]
            hist = np.histogram(cells, bins=bins, range=r, weights=weights)[0]
            wgh[8*j + 32*i:8*j + 32*i+8] = np.reshape(hist, (8, 1))
...
def get_feat_vec(
    ...
    hist = get_gradient_histogram_vec_from_patch(window_magnitudes=window_magnitudes, window_orientations=window_orientations)
    fv = hist / np.linalg.norm(hist)
    fv = np.sqrt(fv)
    # fv = np.multiply(fv, fv>=0.2)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return fv
...
def get_SIFT_descriptors(
    ...
    for i, coordinates in enumerate(zip(Y, X)):
        fvs[i, :] = get_feat_vec(coordinates[1], coordinates[0], magnitudes=magnitudes, orientations=orientations)[:, 0]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return fvs
