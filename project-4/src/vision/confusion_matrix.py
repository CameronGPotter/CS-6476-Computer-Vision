...
def generate_confusion_data(
    ...
    keys = list(label_to_idx.keys())
    # values = list(label_to_idx.values())
    if cuda:
        model.cuda()
    for i, tup in enumerate(loader.dataset):
        img, label = torch.unsqueeze(tup[0], dim=0), tup[1]
        if cuda:
...
def generate_confusion_matrix(
    ...
    for target, prediction in zip(targets, preds):
        ...
        confusion_matrix[target, prediction] += 1.0
...
def generate_accuracy_table(
    ...
    accuracy_table = np.divide(np.sum(targets==preds, axis=0), preds.shape[0])
