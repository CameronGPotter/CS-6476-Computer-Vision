...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        k = kernel_np.shape[0]
        c = self.n_channels
        tile_shape = np.ones(c).astype(int)
        tile_shape[0] = c
        kernel_np = np.tile(kernel_np, tile_shape)
        kernel_np = np.resize(kernel_np, (c, 1, k, k))
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        filtered_image = F.conv2d(x, kernel, padding=(k//2,j//2), groups=self.n_channels)
