...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    mean = ksize // 2
    inv_sqrt_pi = 1 / np.sqrt(2 * np.pi)
    exp_terms = -1 * (1 / (2 * sigma**2))
    
    gaussian_pdf = lambda x: (inv_sqrt_pi / sigma) * np.exp(exp_terms * (x - mean)**2)
    kernel = np.fromfunction(gaussian_pdf, (ksize,))
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    padded = np.pad(image, pad_width=((k//2, k//2), (j//2, j//2), (0, 0)))
    filter = np.repeat(filter[:, :, np.newaxis], c, axis=2)

    filtered_image = np.zeros((m, n, c))

    for row in range(0, m):
      for col in range(0, n):
        filtered_image[row, col, :] = np.sum(np.multiply(padded[row:row+k,col:col+j,:], filter), axis=(0, 1))
